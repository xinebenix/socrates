...
| The first question appears, you answer it, and the second one makes you wait | on a concept with nothing banked the first question must be written before it is shown — that wait is the floor. A *second* wait means the answer path started a generation the session-start warm already had in flight | fixed: the runner now waits on the in-flight generation instead of duplicating it, and items are persisted as each validation passes rather than after a whole set. If it recurs, `ops.recent` shows `buffer.session_fill` — more than one per session with items already banked is the bug |
